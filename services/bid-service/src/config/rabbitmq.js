...
const connectPublisher = async () => {
  try {
    ...
    console.log('✅ Bid Service: RabbitMQ publisher connected');

    connection.on('error', (error) => {
      console.error('❌ Bid Service RabbitMQ error:', error.message);
    });

    connection.on('close', () => {
      console.warn('⚠️  Bid Service RabbitMQ closed. Reconnecting in 5s...');
      setTimeout(connectPublisher, 5000);
    });

  } catch (error) {
    console.error('❌ Bid Service RabbitMQ connection failed:', error.message);
    setTimeout(connectPublisher, 5000);
  }
};
